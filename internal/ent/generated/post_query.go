...
type PostQuery struct {
	...
	withNamedCategories map[string]*PostCategoryQuery
...
// the "saved_by" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PostQuery) WithSavedBy(opts ...func(*UserQuery)) *PostQuery {
	query := (&UserClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withSavedBy = query
	return pq
}
...
func (pq *PostQuery) prepareQuery(ctx context.Context) error {
	...
	if post.Policy == nil {
		return errors.New("generated: uninitialized post.Policy (forgotten import generated/runtime?)")
	}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withSavedBy; query != nil {
		if err := pq.loadSavedBy(ctx, query, nodes,
			...
			func(n *Post, e *User) { n.Edges.SavedBy = append(n.Edges.SavedBy, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range pq.withNamedSavedBy {
		if err := pq.loadSavedBy(ctx, query, nodes,
			func(n *Post) { n.appendNamedSavedBy(name) },
			func(n *Post, e *User) { n.appendNamedSavedBy(name, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range pq.withNamedCategories {
		if err := pq.loadCategories(ctx, query, nodes,
			...
			func(n *Post, e *PostCategory) { n.appendNamedCategories(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *PostQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Post, init func(*Post), assign func(*Post, *User)) error {
...
func (pq *PostQuery) loadSavedBy(ctx context.Context, query *UserQuery, nodes []*Post, init func(*Post), assign func(*Post, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(post.SavedByTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(post.SavedByPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(post.SavedByPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(post.SavedByPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "saved_by" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (pq *PostQuery) loadCategories(ctx context.Context, query *PostCategoryQuery, nodes []*Post, init func(*Post), assign func(*Post, *PostCategory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (pq *PostQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withOwner != nil {
			_spec.Node.AddColumnOnce(post.FieldOwnerID)
		}
	}
...
// WithNamedSavedBy tells the query-builder to eager-load the nodes that are connected to the "saved_by"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (pq *PostQuery) WithNamedSavedBy(name string, opts ...func(*UserQuery)) *PostQuery {
	...
	if pq.withNamedSavedBy == nil {
		pq.withNamedSavedBy = make(map[string]*UserQuery)
	}
	pq.withNamedSavedBy[name] = query
	return pq
}
...
func (pq *PostQuery) WithNamedCategories(name string, opts ...func(*PostCategoryQuery)) *PostQuery {
	...
	if pq.withNamedCategories == nil {
		pq.withNamedCategories = make(map[string]*PostCategoryQuery)
	}
