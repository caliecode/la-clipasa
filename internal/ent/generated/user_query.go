...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withSavedPosts:     uq.withSavedPosts.Clone(),
...
// WithSavedPosts tells the query-builder to eager-load the nodes that are connected to
// the "saved_posts" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithSavedPosts(opts ...func(*PostQuery)) *UserQuery {
	query := (&PostClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withSavedPosts = query
	return uq
}
...
// WithPublishedPosts tells the query-builder to eager-load the nodes that are connected to
// the "published_posts" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithPublishedPosts(opts ...func(*PostQuery)) *UserQuery {
	query := (&PostClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withPublishedPosts = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withSavedPosts; query != nil {
		if err := uq.loadSavedPosts(ctx, query, nodes,
			func(n *User) { n.Edges.SavedPosts = []*Post{} },
			func(n *User, e *Post) { n.Edges.SavedPosts = append(n.Edges.SavedPosts, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := uq.withPublishedPosts; query != nil {
		if err := uq.loadPublishedPosts(ctx, query, nodes,
			func(n *User) { n.Edges.PublishedPosts = []*Post{} },
			func(n *User, e *Post) { n.Edges.PublishedPosts = append(n.Edges.PublishedPosts, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range uq.withNamedSavedPosts {
		if err := uq.loadSavedPosts(ctx, query, nodes,
			...
			func(n *User, e *Post) { n.appendNamedSavedPosts(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range uq.withNamedLikedPosts {
		if err := uq.loadLikedPosts(ctx, query, nodes,
			func(n *User) { n.appendNamedLikedPosts(name) },
			func(n *User, e *Post) { n.appendNamedLikedPosts(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range uq.withNamedPublishedPosts {
		if err := uq.loadPublishedPosts(ctx, query, nodes,
			func(n *User) { n.appendNamedPublishedPosts(name) },
			func(n *User, e *Post) { n.appendNamedPublishedPosts(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadSavedPosts(ctx context.Context, query *PostQuery, nodes []*User, init func(*User), assign func(*User, *Post)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.SavedPostsTable)
		s.Join(joinT).On(s.C(post.FieldID), joinT.C(user.SavedPostsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.SavedPostsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.SavedPostsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "saved_posts" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (uq *UserQuery) loadPublishedPosts(ctx context.Context, query *PostQuery, nodes []*User, init func(*User), assign func(*User, *Post)) error {
	...
	query.Where(predicate.Post(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.PublishedPostsColumn), fks...))
	}))
...
func (uq *UserQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*User, init func(*User), assign func(*User, *Comment)) error {
	...
	query.withFKs = true
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(comment.FieldOwnerID)
	}
...
// WithNamedSavedPosts tells the query-builder to eager-load the nodes that are connected to the "saved_posts"
...
func (uq *UserQuery) WithNamedSavedPosts(name string, opts ...func(*PostQuery)) *UserQuery {
	...
	if uq.withNamedSavedPosts == nil {
		uq.withNamedSavedPosts = make(map[string]*PostQuery)
	}
	uq.withNamedSavedPosts[name] = query
	return uq
}

// WithNamedLikedPosts tells the query-builder to eager-load the nodes that are connected to the "liked_posts"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithNamedLikedPosts(name string, opts ...func(*PostQuery)) *UserQuery {
	...
	if uq.withNamedLikedPosts == nil {
		uq.withNamedLikedPosts = make(map[string]*PostQuery)
	}
	uq.withNamedLikedPosts[name] = query
	return uq
}

// WithNamedPublishedPosts tells the query-builder to eager-load the nodes that are connected to the "published_posts"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithNamedPublishedPosts(name string, opts ...func(*PostQuery)) *UserQuery {
	...
	if uq.withNamedPublishedPosts == nil {
		uq.withNamedPublishedPosts = make(map[string]*PostQuery)
	}
	uq.withNamedPublishedPosts[name] = query
	return uq
}
