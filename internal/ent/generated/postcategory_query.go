...
func (pcq *PostCategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PostCategory, error) {
	...
	if query := pcq.withPost; query != nil {
		if err := pcq.loadPost(ctx, query, nodes, nil,
			func(n *PostCategory, e *Post) { n.Edges.Post = e }); err != nil {
			return nil, err
		}
	}
...
func (pcq *PostCategoryQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*PostCategory, init func(*PostCategory), assign func(*PostCategory, *Post)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*PostCategory)
	for i := range nodes {
		if nodes[i].post_categories == nil {
			continue
		}
		fk := *nodes[i].post_categories
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "post_categories" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
