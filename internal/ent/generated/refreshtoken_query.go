...
func (rtq *RefreshTokenQuery) prepareQuery(ctx context.Context) error {
	...
	if refreshtoken.Policy == nil {
		return errors.New("generated: uninitialized refreshtoken.Policy (forgotten import generated/runtime?)")
	}
	if err := refreshtoken.Policy.EvalQuery(ctx, rtq); err != nil {
		return err
	}
	return nil
}
...
func (rtq *RefreshTokenQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := rtq.ctx.Fields; len(fields) > 0 {
		...
		if rtq.withOwner != nil {
			_spec.Node.AddColumnOnce(refreshtoken.FieldOwnerID)
		}
	}
...
func (rtq *RefreshTokenQuery) ForUpdate(opts ...sql.LockOption) *RefreshTokenQuery {
	if rtq.driver.Dialect() == dialect.Postgres {
		rtq.Unique(false)
	}
	rtq.modifiers = append(rtq.modifiers, func(s *sql.Selector) {
		s.ForUpdate(opts...)
	})
	return rtq
}
