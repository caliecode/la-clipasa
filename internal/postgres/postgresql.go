...
func New(logger *zap.SugaredLogger, options ...Option) (*pgxpool.Pool, *sql.DB, error) {
	...
	if err != nil {
		return nil, nil, internal.WrapErrorf(err, internal.ErrorCodeUnknown, "pgxpool.NewWithConfig")
	}

	if err := pgxPool.Ping(context.Background()); err != nil {
		return nil, nil, internal.WrapErrorf(err, internal.ErrorCodeUnknown, "pgxPool.Ping")
	}
